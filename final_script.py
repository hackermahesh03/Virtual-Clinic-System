...
def make_final_paper():
    ...
    temp_body_path = r'C:\Users\DEll\OneDrive\Documents\PAPER\temp_body_win32.docx'
    ...
    try:
        # 1. Clean the body file (remove duplicate Title, Abstract, Keywords)
        doc_body = word.Documents.Open(temp_body_path)
        # Delete first 3 paragraphs
        ...
        # 2. Open the final document and fill placeholders
        doc = word.Documents.Open(final_path)
        
        # Headers & Footers
        replace_text(doc, '<Journal Name>', 'Engineering Science and Technology, an International Journal')
        replace_text(doc, 'Author Name', 'Roopa Tirumalasetti, Mahesh Babu Nettem, Pavan Reddy Narva, Siva Karthik Valiveti')
        ...
        # Keywords
        replace_text(doc, 'Keyword_1', 'COVID-19 detection')
        replace_text(doc, 'Keyword_2', 'chest X-ray classification')
        replace_text(doc, 'Keyword_3', 'test-time augmentation')
        replace_text(doc, 'Keyword_4', 'contrast enhancement')
        replace_text(doc, 'Keyword_5', 'bias correction')
